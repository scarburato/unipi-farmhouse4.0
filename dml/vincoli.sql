...
/***** Lista giornate *****/
CREATE TRIGGER `chk_days`
BEFORE INSERT ON `Somministrazione` FOR EACH ROW
BEGIN
    DECLARE i       TINYINT UNSIGNED DEFAULT 0;
    DECLARE commaN  TINYINT UNSIGNED DEFAULT 0;
    
    -- Numero di virgole nella stringa
    SET commaN = LENGTH(NEW.`lista giornate senza somministrazione`) - LENGTH(REPLACE(`lista giornate senza somministrazione`, ',', ''));
    
    IF commaN >= 6 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Il medicinale deve essere somministrato almeno una volta a settimana!';
    END IF;
    
    WHILE i < commaN DO
        IF 
            SUBSTRING_INDEX(SUBSTRING_INDEX(NEW.`lista giornate senza somministrazione`, ',', i + 1), ',', -1) 
                NOT IN ('Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato')
        THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Inserito giorno non valido!';
        END IF;
    ...
    -- Numero di virgole nella stringa
    SET commaN = LENGTH(NEW.`lista orari somministrazione`) - LENGTH(REPLACE(`lista orari somministrazione`, ',', ''));
